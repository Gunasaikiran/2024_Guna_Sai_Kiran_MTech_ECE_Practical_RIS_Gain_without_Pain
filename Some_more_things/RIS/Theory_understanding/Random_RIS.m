...
tot_time_slots = 200000000;      % Total time slots
Number_of_setups = 4;        % Monte Carlo iterations

Ts_values = [200, 800, 1800, 3000];            % IRS switching intervals
tau_values = [2, 50, 100, 2000, 10000, 20000,200000,2000000]; % PF averaging parameters
...
parfor setup = 1:Number_of_setups
    ...
    for ts_idx = 1:length(Ts_values)
        ...
        for tau_idx = 1:length(tau_values)
            ...
            %print_text_1 = sprintf('setup value = %d, Ts value = %d, Tc value = %d, slot no = %d',setup,Ts,tau,);
            ...
            for t = 1:tot_time_slots
                if mod(t - 1, Ts) == 0
                      IRS_state = randi(K); % randomly select one of the K user configs
                end
                IRS_config_diag = IRS_config_user{IRS_state};
                print_text_1 = sprintf('setup value = %d, Ts value = %d, Tc value = %d, slot no = %d',setup,Ts,tau,t);
...
%% --- Plot: Average Rate vs τ for one Ts value ---
figure;
...
legend_labels = [ ...
    arrayfun(@(x) sprintf('PF + Random RIS (T_s = %d s)', x), Ts_values, 'UniformOutput', false), ...
    {'Round-Robin + Optimal RIS (Benchmark)'} ...
];
 
legend(legend_labels, 'Location', 'best');
title('Convergence of PF Throughput to Optimal Value for Large T_c (Any Finite T_s)');
xlabel('Averaging Window T_c');
ylabel('Achieved Throughput (Mbps)');
grid on;
grid on;
xticks(tau_values);
xticklabels(string(tau_values));
saveas(gcf, 'PF_Rate_vs_Tau_Random_IRS_Log_upd.fig');
