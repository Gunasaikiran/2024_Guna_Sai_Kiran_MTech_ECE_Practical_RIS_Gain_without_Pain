...
Ts_values = [1, 25000, 50000];
tau_values = [2, 50, 1000, 5000];%100, 250, 500,
...
for setup = 1:Number_of_setups
    ...
    %% 1. PF Scheduling with Fixed Random IRS (varying Ts and tau)
    ...
    %% 1b. Benchmark: Round-Robin with Same Random IRS Config
    rate_rr_random = zeros(1,K);
    for k = 1:K
        h_rr = h_BS_UE(k) + (h_IRS_UE(:,k).') * IRS_config_diag * h_BS_IRS;
        rate_rr_random(k) = log2(1 + (abs(h_rr)^2)*txt_SNR_lin);
    end
    Rate_benchmark_RR_Random_IRS(setup) = mean(rate_rr_random);

    %% 2. Benchmark: Round-robin with optimized IRS (search over DFT columns)
    rate_bf = zeros(1,K);
    for k = 1:K
        best_rate = 0;
        for l = 1:N
            IRS_config_opt = diag(DFT_codebook(:, l));
            h_opt = h_BS_UE(k) + (h_IRS_UE(:,k).') * IRS_config_opt * h_BS_IRS;
    ...
    Rate_benchmark_IRS(setup) = mean(rate_bf);

    %% 3. Benchmark: LOS only (no IRS)
