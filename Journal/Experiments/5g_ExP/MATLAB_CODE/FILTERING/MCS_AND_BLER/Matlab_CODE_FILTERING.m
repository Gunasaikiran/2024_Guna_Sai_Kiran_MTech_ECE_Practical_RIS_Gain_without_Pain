...
destDir   = '/home/gunasaikiran/Desktop/5g_ExP/MATLAB_CODE/FILTERING/MCS_AND_BLER'; 

% Define file names
inputFile = fullfile(sourceDir, 'BOTH_UES.log');
outputFile = fullfile(destDir, 'MCS_filtered_UE_2.txt');
...
while ~feof(fid_in)
    line = fgetl(fid_in);
    if  contains(line, 'UE d5aa: dlsch_rounds') 
         fprintf(fid_out, '%s\n', line);
    end
